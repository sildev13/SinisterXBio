...
export default function About() {
  const structure = [
    ...
    {
      title: about.devices.title, // หัวข้อ My Devices ใน Sidebar
      display: about.devices.display,
      items: about.devices.items.map((item) => item.name),
    },
    {
    ...
    <Column maxWidth="m">
      ...
      <Row fillWidth s={{ direction: "column" }} horizontal="center">
        ...
        <Column className={styles.blockAlign} flex={9} maxWidth={40}>
          ...
          {/* --- เพิ่ม Section: My Devices ตรงนี้ --- */}
          {about.devices.display && (
            <>
              <Heading as="h2" id={about.devices.title} variant="display-strong-s" marginBottom="m">
                {about.devices.title}
              </Heading>
              <Column fillWidth gap="l" marginBottom="40">
                {about.devices.items.map((item, index) => (
            ...
            <>
              ...
              <Column fillWidth gap="l">
                {about.technical.skills.map((skill, index) => (
                  <Column key={`${skill.title}-${index}`} fillWidth gap="4" marginBottom="m">
